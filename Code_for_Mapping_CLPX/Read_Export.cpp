...
void Mapping_Fsnow_CLPX();
void Mapping_Fsnow_CLPX_using_SnowDepth();
...
void Mapping_Snow_Status_CLPX();

void Processing_Images();
void Put_several_image_stripes_into_one_image_file();
...
void Output_a_mutiband_image_to_individual_Images_Pixel();
...
void Output_pixel_values_of_DirectModelled_Mapped_SnowDepth_images();

void Output_pixel_values_of_SDD_images();
void Output_pixel_values_of_modeled_SDD_images();
...
void Output_Median_or_Avg_values_for_a_set_of_Images();
...
#define PIXELS 458   //HV:113, 680    CLPX:458,       
#define LINES  370   //HV:513, 3080   CLPX:370,  
...
//	Mapping_Fsnow_CLPX();  
//	Mapping_Fsnow_CLPX_using_SnowDepth();

	Processing_Images();

//	Mapping_Snow_Status_CLPX();

//	Output_Median_or_Avg_values_for_a_set_of_Images();
...
// Output_a_mutiband_image_to_individual_Images();   //seperated by band
// Output_a_mutiband_image_to_individual_Images_Pixel();  //seperated by pixel
...
//	Output_pixel_values_of_DirectModelled_Mapped_SnowDepth_images();

//	Output_pixel_values_of_SDD_images();  //Mitchell's data 32f
	Output_pixel_values_of_modeled_SDD_images();   //short


//	Correct_Fsnow_and_generate_Fsnow_for_each_Year();

//	Put_several_image_stripes_into_one_image_file();
}
...
void Correct_Fsnow_and_generate_Fsnow_for_each_Year() {
	...
	char Path_In_Mask[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026May\\";
	char Path_In_SnowEnd_u[] = "E:\\Snow_Alaska\\Data_From_Mitchell\\2026_YZ\\CLPX_SnowEndB_Layers\\";
	char Path_In[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026May\\";
	char Path_Out[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026May\\";
...
//Ob_precip
//	double Modelled_SnowStatus_1[7] = { 53.34,55.88,48.26,68.58,78.74,83.82,88.9}, //2019-2025, Total_Previous_Oct_to_March
//		Modelled_SnowStatus_2[7] = { 63.5,60.96,60.96,86.36,35.56,58.42,100.06 };  //2012-2018,including 2014  
//ERA5_precip
	double Modelled_SnowStatus_1[7] = { 104.89,103.7,92.95,102.15,125.16,134.1,155.91}, //2019-2025, Total_Previous_Oct_to_March
		Modelled_SnowStatus_2[7] = { 123.43,122.11,144.38,130.06,58.4,112.48,183.63};  //2012-2018, including 2014 
	...
	for (i = 0; i < 6; i++) {
		...
		sprintf(String, "%sCLPX_ERA5_precip_Fsnow_estimated_from_7yr_avg_Fsnow_%d.dat", Path_Out, Year);
...
// input and calculate ER
	sprintf(String, "%sCLPX_study_area_excluding_water_road.raw", Path_In_Mask);  //
	...
	for (i = 0; i < 7; i++) {
		Year = 2019 + i;
		sprintf(String, "%sCLPX_ERA5_precip_avg_Fsnow_%d.dat", Path_In, Year);
		if ((fpt_In_16S[i] = fopen(String, "rb")) == NULL) {
			printf("Cannot open file %s\n", String);
			exit(0);
		}
		sprintf(String, "%sCLPX_SnowEnd_u_%d.raw", Path_In_SnowEnd_u, Year);
		if ((fpt_In_32R[i] = fopen(String, "rb")) == NULL) {
			printf("Cannot open file %s\n", String);
			exit(0);
		}
	}
	sprintf(String, "%sCLPX_ERA5_precip_avg_Fsnow_7yr_avg.dat", Path_In);
...
//	char Path[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026July\\";    
//	char Path[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_SnowDepth_2026July\\";
	char Path[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Dates\\";
//	char Path[] =  "E:\\Snow_Alaska\\Modelling_2026\\Model_Input_Output\\CLPX_DirectMapping\\";
	char Note[300];

	int NN = 7;  //Fsnow:7, Ob_snowDepth:6

	int Year[7] = { 2019, 2020, 2021, 2022, 2023, 2024, 2025 };
//	int Year[6] = { 2012, 2013, 2015, 2016, 2017, 2018 };  
//	int Year[7] = { 2012, 2013, 2014, 2015, 2016, 2017, 2018 };
	int N;
...
//	sprintf(Note, "%sCLPX_Ob_precip_avg_Fsnow_2019_2025.dat", Path);  //Ob, ERA5
//	sprintf(Note, "%sCLPX_Ob_precip_SnowDepth_Apr_avg_Fsnow_avg.dat", Path);  //

//	sprintf(Note, "%sCLPX_Ob_precip_DirectMapping_SnowDepth_2012_2018.dat", Path);  //
	sprintf(Note, "%sCLPX_ERA5_precip_SDD_mid_Fsnow_2019_2025.dat", Path);  //
...
//		sprintf(Note, "%sCLPX_Ob_precip_avg_Fsnow_%d.dat", Path, Year[N]);  //Ob, ERA5
//		sprintf(Note, "%sCLPX_Ob_precip_Fsnow_from_SnowDepth_%d.dat", Path, Year[N]);  //
//		sprintf(Note, "%sCLPX_Ob_precip_SnowDepth_avg_Fsnow_Apr_%d.dat", Path, Year[N]);  //Ob, ERA5

//		sprintf(Note, "%sCLPX_Ob_precip_DirectMapping_SnowDepth_Apr_%d.dat", Path, Year[N]);  //direct mapping
		sprintf(Note, "%sCLPX_ERA5_precip_SDD_mid_Fsnow_%d.dat", Path, Year[N]);  //
...
//bands seperated by pixel
	...
	int Line, Pix;
	short Data_All_Years[7];
	short* Line_x_0 = new short[PIXELS], * Line_x_1 = new short[PIXELS], * Line_x_2 = new short[PIXELS],
		* Line_x_3 = new short[PIXELS], * Line_x_4 = new short[PIXELS], * Line_x_5 = new short[PIXELS],
		* Line_x_6 = new short[PIXELS];

	FILE* fpt_Input, * fpt_Output[7];

	//	sprintf(Note, "%sCLPX_Ob_precip_avg_Fsnow_2019_2025.dat", Path);  //Ob, ERA5
	//	sprintf(Note, "%sCLPX_ERA5_precip_Fsnow_from_SnowDepth_2012_2018.dat", Path);  //Ob, ERA5
	//	sprintf(Note, "%sCLPX_ERA5_precip_SnowDepth_Apr_avg_Fsnow_avg.dat", Path);  //
	...
	for (N = 0; N < NN; N++) {
		...
		if ((fpt_Output[N] = fopen(Note, "wb")) == NULL) {
			printf("Cannot open output file %s\n", Note);
			exit(0);
		}
	}
	...
	for (Line = 0; Line < LINES; Line++) {
		for (Pix = 0; Pix < PIXELS; Pix++) {
			fread(Data_All_Years, 2, NN, fpt_Input);

			Line_x_0[Pix] = Data_All_Years[0]; Line_x_1[Pix] = Data_All_Years[1]; Line_x_2[Pix] = Data_All_Years[2];
			Line_x_3[Pix] = Data_All_Years[3]; Line_x_4[Pix] = Data_All_Years[4]; Line_x_5[Pix] = Data_All_Years[5];
			Line_x_6[Pix] = Data_All_Years[6];
		}  //Pix

		fwrite(Line_x_0, 2, PIXELS, fpt_Output[0]);  fwrite(Line_x_1, 2, PIXELS, fpt_Output[1]);
		fwrite(Line_x_2, 2, PIXELS, fpt_Output[2]);  fwrite(Line_x_3, 2, PIXELS, fpt_Output[3]);
		fwrite(Line_x_4, 2, PIXELS, fpt_Output[4]);  fwrite(Line_x_5, 2, PIXELS, fpt_Output[5]);
		fwrite(Line_x_6, 2, PIXELS, fpt_Output[6]);
	}  //Line

	for (N = 0; N < NN; N++) fclose(fpt_Output[N]);
	fclose(fpt_Input);

	delete[]Line_x_0;  delete[]Line_x_1;  delete[]Line_x_2;
	delete[]Line_x_3;  delete[]Line_x_4;  delete[]Line_x_5;
	delete[]Line_x_6;
}
...
void Put_several_image_stripes_into_one_image_file() {
	...
	short* Line_x = new short[PIXELS*NN];

	FILE* fpt_Input, * fpt_Output;

	sprintf(Note, "%sCLPX_Ob_precip_DirectMapping_SDD1_2019_2025.dat", Path);  //
//	sprintf(Note, "%sCLPX_Ob_precip_DirectMapping_SnowDepth_2012_2018.dat", Path);  //
	if ((fpt_Output = fopen(Note, "wb")) == NULL) {
		printf("Cannot open output file %s\n", Note);
		exit(0);
	...
	Line1 = 0;
	Line2 = 140;
	sprintf(Note, "%sCLPX_Ob_precip_DirectMapping_SDD1_2019_2025_Line_%d_%d.dat", Path, Line1, Line2);  //
//	sprintf(Note, "%sCLPX_Ob_precip_DirectMapping_SnowDepth_2012_2018_Line_%d_%d.dat", Path, Line1, Line2);  
	if ((fpt_Input = fopen(Note, "rb")) == NULL) {
		printf("Cannot open output file %s\n", Note);
		exit(0);
	}
	for (Line = 0; Line < LINES; Line++) {
		fread(Line_x, 2, PIXELS*NN, fpt_Input);
		if(Line >= Line1 && Line <= Line2) fwrite(Line_x, 2, PIXELS*NN, fpt_Output);
	}  //Line
	fclose(fpt_Input);
...
//	char Path[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_SnowDepth_2026July\\";
	...
	int Line, Pix, V[14], N[14];

	double avg[14], avg_Observed[14], diff[14], abs_diff[14];
	int i, j;
...
//	sprintf(String, "%sSnowDepth_Ob_precip_ERA5_precip.txt", Path);  //two modelled snow depth
//	sprintf(String, "%sSnowDepth_Ob_Ob_precip.txt", Path);  //  ERA5, Ob

	sprintf(String, "%sSnowDepth_ERA5_precip_DirectlyModelled_and_Mapped.txt", Path);
	...
	for (j = 0; j < N_rows; j++) {
		...
		for (i = 0; i < N_cols_half; i++) {
			if (V[i] > -100 && V[N_cols_half + i] > -100) {
				N[i]++;
				avg_Observed[i] += V[i];
				avg[i] += V[i + N_cols_half];  //modelled 
	...
	fprintf(fpt_Output, "Pixels ");
	for (i = 0; i < N_cols_half; i++) fprintf(fpt_Output, "%d ", N[i]);   //N
	fprintf(fpt_Output, "\n");

	fprintf(fpt_Output, "Observed_avg ");
	for (i = 0; i < N_cols_half; i++) fprintf(fpt_Output, "%lf ", avg_Observed[i] * 0.1);  //mean observed
	fprintf(fpt_Output, "\n");

	fprintf(fpt_Output, "Modeled_avg ");
	for (i = 0; i < N_cols_half; i++) fprintf(fpt_Output, "%lf ", avg[i] * 0.1);  //mean modelled
	fprintf(fpt_Output, "\n");

	fprintf(fpt_Output, "Ratio_Modeled_Observed ");
	for (i = 0; i < N_cols_half; i++) fprintf(fpt_Output, "%lf ", avg[i]/ avg_Observed[i]);  //ratio
	fprintf(fpt_Output, "\n");

	fprintf(fpt_Output, "Diff_Modeled_Observed ");
	for (i = 0; i < N_cols_half; i++) fprintf(fpt_Output, "%lf ", diff[i] * 0.1); //
	fprintf(fpt_Output, "\n");

	fprintf(fpt_Output, "abs_Diff_Modeled_Observed ");
	for (i = 0; i < N_cols_half; i++) fprintf(fpt_Output, "%lf ", abs_diff[i] * 0.1);
	fprintf(fpt_Output, "\n");

	fprintf(fpt_Output, "relative_abs_Diff_Modeled_Observed ");
	for (i = 0; i < N_cols_half; i++) fprintf(fpt_Output, "%lf ", abs_diff[i] / avg[i] * 100);
	fprintf(fpt_Output, "\n");
...
void Calculate_mean_and_differences_Fsnow() { 
	...
	sprintf(String, "%sCLPX_Ob_precip_avg_Fsnow_&_7yr_avg.txt", Path);  //
//	sprintf(String, "%sHV_Fsnow_with_7yr_avg.txt", Path);  //
	...
	for (i = 0; i < N_cols_1; i++) fprintf(fpt_Output, "%lf ", avg[i]*0.001);
	fprintf(fpt_Output, "\n");

	for (i = 0; i < N_cols_1; i++) fprintf(fpt_Output, "%lf ", avg[i]/ avg_last_col[i]);  //ratio
	fprintf(fpt_Output, "\n");
...
void Fsnow_from_SnowDepth_caluclate_avg_or_Median() {
	...
	char Path_Fsnow[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026May\\";
	char Clime_Site_Name[] = "CLPX_ERA5_precip";
	char Note[300];
...
void Fsnow_caluclate_mean_relative_absolute_difference_from_avg() {
	...
	char Path_Fsnow[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026July\\";
	char Path_SnowEnd_u[] = "E:\\Snow_Alaska\\Data_From_Mitchell\\2026_YZ\\CLPX_SnowEndB_Layers\\";
	char Clime_Site_Name[] = "CLPX_Ob_precip";  //Ob, ERA5
	char Note[300];
	...
	for (i = 0; i < YEARS; i++) {
		...
		sprintf(Note, "%sCLPX_SnowEnd_u_%d.raw", Path_SnowEnd_u, Year);  //
...
void Mapping_Snow_Status_CLPX()
{
	...
	char Path_In_Area[]  = "E:\\Snow_Alaska\\CLPX\\CLPX_30m\\";                    
	char Path_In_Fsnow[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026July\\";
	char Path_Out_Area[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Dates\\";
	char Clim_Site_Name[] = "CLPX_ERA5_Precip";  //Ob, ERA5
	...
	int Year;
	int Num_Input_Values = 7;  //for observation: 7 years from 2012 to 2018 (2014 skipped when mapping)
...
//	sprintf(Note, "%s%s_SnowDepth_Apr_avg_Fsnow_avg.dat", Path_Out_Area, Clim_Site_Name);
	sprintf(Note, "%s%s_SDD_mid_Fsnow_2019_2025.dat", Path_Out_Area, Clim_Site_Name);
	if ((fpt_Output = fopen(Note, "wb")) == NULL) {
		printf("Cannot open output file %s\n", Note);
		exit(0);
	}

	for (Year = 2019; Year <= 2025; Year++) {  //2012 to 2018 for Ob years, 6 yrs
...
//       sprintf(File_Name, "%s%s_%s_SnowDepth_Apr_ObYears", Path, Clim_Site_Name, CoverType_Name[Type]);
        ...
        sprintf(File_Name, "%s%s_%s_EndDate_Mono_July7", Path, Clim_Site_Name, CoverType_Name[Type]);
...
//Mapping: open image files  --------------------------------
		sprintf(Note, "%sCLPX_30m_Landcover.raw", Path_In_Area);  //8U, _5m
		//	sprintf(Note, "%sHV_30m_Landcover.raw", Path_In_Area);  //8U, HV_5m
		if ((fpt_LC = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
		}
		sprintf(Note, "%sCLPX_30m_LAI.raw", Path_In_Area);  //16S, LAI*1000,  _5m
		if ((fpt_LAI = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
		}
		sprintf(Note, "%sCLPX_30m_ArcticDEM_Slope.raw", Path_In_Area);  //32R, degrees, _5m, 30m
		//	sprintf(Note, "%sHV_5m_Slope.raw", Path_In_Area);  //32R, degrees
		if ((fpt_Slope = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
		}
		sprintf(Note, "%sCLPX_30m_ArcticDEM_Aspect.raw", Path_In_Area);  //32R, degrees, _5m, 30m
		//	sprintf(Note, "%sHV_5m_Aspect.raw", Path_In_Area);  //32R, degrees
		if ((fpt_Aspect = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
...
//		sprintf(Note, "%s%s_avg_Fsnow_7yr_avg.dat", Path_In_Fsnow, Clim_Site_Name);  //16S, avg
		sprintf(Note, "%s%s_mid_Fsnow_%d.dat", Path_In_Fsnow, Clim_Site_Name, Year);  //16S, individual year
...
void Mapping_Snow_Status_HV()
{
	...
	//						0.858,0.920,1.308 };  //ratio of snowfall from Nov to March to average from 2001 to 2023
...
void Output_Median_or_Avg_values_for_a_set_of_Images()
{
	char Path_In[] = "E:\\Snow_Alaska\\Modelling\\HV\\HV_30m_Spatial_Output\\";                //HV, CLPX       
	char Path_Out[] = "E:\\Snow_Alaska\\Modelling\\HV\\HV_30m_Spatial_Output\\Analysis\\";     //HV, CLPX
	char String[200];
	...
	int yy, Line, Pix;
	float All_Fsnow[6], Fsnow;

	short *aLine_In = new short[PIXELS],
		  *aLine_Out = new short[PIXELS],
		  *All_Lines = new short[PIXELS*N_Images];

	FILE* fpt_Out, * fpt_In[6];

	sprintf(String, "%sHV_30m_Fsnow_avg_%d_%d.dat", Path_Out, Year[0], Year[N_Images-1]);  //CLPX
	if ((fpt_Out = fopen(String, "wb")) == NULL) {
		printf("Cannot open file %s\n", String);
		exit(0);
	}

	for (yy = 0; yy < N_Images; yy++) {  
		printf("%d ", Year[yy]);

		sprintf(String, "%sHV_30m_Fsnow_%d.dat", Path_In, Year[yy]);  //HV, CLPX
		if ((fpt_In[yy] = fopen(String, "rb")) == NULL) {
			printf("Cannot open file %s\n", String);
			exit(0);
		}
	}
	...
	for (Line = 0; Line < LINES; Line++) {
		if ((Line % 10) == 0) printf("%d ", Line);

			N = 0;
			for (yy = 0; yy < N_Images; yy++) {
				fread(aLine_In, 2, PIXELS, fpt_In[yy]);

				for (Pix = 0; Pix < PIXELS; Pix++) {
					All_Lines[N] = aLine_In[Pix];
					N++;
				}
			}
			...
			for (Pix = 0; Pix < PIXELS; Pix++) {
				...
				if (Fsnow > 1199) aLine_Out[Pix] = 1200;
				aLine_Out[Pix] = short(Fsnow + 0.5);
			}

			fwrite(aLine_Out, 2, PIXELS, fpt_Out);
	}
	for (yy = 0; yy < N_Images; yy++) fclose(fpt_In[yy]);
	fclose(fpt_Out);
...
void Output_pixel_values_of_one_image()
{
	char Path_In_Mask[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026July\\";
	char Path_In[]  = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026July\\";
	char Path_Out[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026July\\";
	...
	//output file
	sprintf(String, "%sOb_precip_avg_Fsnow_7yr_avg.txt", Path_Out);  //Ob, ERA5
	if ((fpt_Out = fopen(String, "w")) == NULL) {
		printf("Cannot open file %s\n", String);
		exit(0);
	}
	fprintf(fpt_Out, "Line Pix Ob_precip_avg_Fsnow_7yr_avg\n");
	...
	sprintf(String, "%sCLPX_Ob_precip_avg_Fsnow_7yr_avg.dat", Path_In);  //Ob, ERA5
...
{
//	char Path_In_Mask[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026May\\";
	char Path_In_Directly_Modelled[] = "E:\\Snow_Alaska\\Modelling_2026\\Model_Input_Output\\CLPX_DirectMapping\\";
	char Path_In[] =  "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_SnowDepth_2026May\\";
	char Path_Out[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_SnowDepth_2026May\\";
	...
	//output file
	sprintf(String, "%sSnowDepth_Ob_precip_DirectlyModelled_and_Mapped.txt", Path_Out);  //ERA5, Ob
	if ((fpt_Out = fopen(String, "w")) == NULL) {
		printf("Cannot open file %s\n", String);
		exit(0);
	}
	fprintf(fpt_Out, "Line Pix ");
	for (yy = 0; yy < N_Files; yy++) fprintf(fpt_Out, "%d_DirectlyModelled ", Year[yy]);
	for (yy = 0; yy < N_Files; yy++) fprintf(fpt_Out, "%d_Mapped ", Year[yy]);  //ERA5, Ob
	fprintf(fpt_Out, "\n");
...
//	}
	...
	for (yy = 0; yy < N_Files; yy++) {
		sprintf(String, "%sCLPX_Ob_precip_DirectMapping_SnowDepth_Apr_%d.dat", Path_In_Directly_Modelled, Year[yy]);
		if ((fpt_In_Ob[yy] = fopen(String, "rb")) == NULL) {
			printf("Cannot open file %s\n", String);
			exit(0);
		}

		sprintf(String, "%sCLPX_Ob_precip_SnowDepth_avg_Fsnow_Apr_%d.dat", Path_In, Year[yy]);  //ERA5, Ob
...
//		fread(aLine_8U, 1, PIXELS, fpt_In_8U);
		...
		for (Pix = 0; Pix < PIXELS; Pix++) {
//			if (aLine_8U[Pix] == 1) {
			if (aLine_Ob_0[Pix] != -200) {  //-200 for no data
...
{
	char Path_In_Mask[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026July\\";
	char Path_In[]      = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Dates\\";
	char Path_Out[]     = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Dates\\"; 
	...
	short* aLine_SDD_0 = new short[PIXELS]; 	short* aLine_SDD_1 = new short[PIXELS];
	short* aLine_SDD_2 = new short[PIXELS]; 	short* aLine_SDD_3 = new short[PIXELS];
	short* aLine_SDD_4 = new short[PIXELS]; 	short* aLine_SDD_5 = new short[PIXELS];
	short* aLine_SDD_6 = new short[PIXELS];

	short SDD[7];

	FILE* fpt_Out, * fpt_In_8U, * fpt_In[7];

	//output file
	sprintf(String, "%sCLPX_ERA5_precip_SDD_mid_Fsnow.txt", Path_Out);
	if ((fpt_Out = fopen(String, "w")) == NULL) {
		printf("Cannot open file %s\n", String);
		exit(0);
	}
	fprintf(fpt_Out, "Line Pix ");
	for (yy = 0; yy < N_Files; yy++) fprintf(fpt_Out, "%d ", Year[yy]);
	fprintf(fpt_Out, "\n");
	...
	for (yy = 0; yy < N_Files; yy++) {
		sprintf(String, "%sCLPX_ERA5_precip_SDD_mid_Fsnow_%d.dat", Path_In, Year[yy]);  //Ob
		if ((fpt_In[yy] = fopen(String, "rb")) == NULL) {
			printf("Cannot open file %s\n", String);
			exit(0);
		}
	...
	for (Line = 0; Line < LINES; Line++) {
		...
		fread(aLine_SDD_0, 2, PIXELS, fpt_In[0]);
		fread(aLine_SDD_1, 2, PIXELS, fpt_In[1]);
		fread(aLine_SDD_2, 2, PIXELS, fpt_In[2]);
		fread(aLine_SDD_3, 2, PIXELS, fpt_In[3]);
		fread(aLine_SDD_4, 2, PIXELS, fpt_In[4]);
		fread(aLine_SDD_5, 2, PIXELS, fpt_In[5]);
		fread(aLine_SDD_6, 2, PIXELS, fpt_In[6]);

		for (Pix = 0; Pix < PIXELS; Pix++) {
			if (aLine_8U[Pix] == 1) {

				SDD[0] = aLine_SDD_0[Pix]; 	SDD[1] = aLine_SDD_1[Pix]; SDD[2] = aLine_SDD_2[Pix];
				SDD[3] = aLine_SDD_3[Pix]; 	SDD[4] = aLine_SDD_4[Pix]; SDD[5] = aLine_SDD_5[Pix];
				SDD[6] = aLine_SDD_6[Pix];

				fprintf(fpt_Out, "%d %d ", Line, Pix);
				for (yy = 0; yy < N_Files; yy++) fprintf(fpt_Out, "%.1lf ", SDD[yy]*0.1);
				fprintf(fpt_Out, "\n");
			}
		}  //pix
	}  //line
	...
	fclose(fpt_Out);
	fclose(fpt_In_8U);
	for (yy = 0; yy < N_Files; yy++) fclose(fpt_In[yy]);

	delete[]aLine_8U;
	delete[]aLine_SDD_0; delete[]aLine_SDD_1; delete[]aLine_SDD_2; delete[]aLine_SDD_3;
	delete[]aLine_SDD_4; delete[]aLine_SDD_5; delete[]aLine_SDD_6;
}
...
{
	char Path_In_Mask[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026May\\";
	char Path_In[]  = "E:\\Snow_Alaska\\Data_From_Mitchell\\2026_YZ\\CLPX_SnowEndB_Layers\\";
	char Path_Out[] = "E:\\Snow_Alaska\\Data_From_Mitchell\\2026_YZ\\CLPX_SnowEndB_Layers\\";
	...
	float* aLine_SDD_0 = new float[PIXELS]; 	float* aLine_SDD_1 = new float[PIXELS];
	float* aLine_SDD_2 = new float[PIXELS]; 	float* aLine_SDD_3 = new float[PIXELS];
	float* aLine_SDD_4 = new float[PIXELS]; 	float* aLine_SDD_5 = new float[PIXELS];
	float* aLine_SDD_6 = new float[PIXELS]; 	
	...
	//output file
	sprintf(String, "%sCLPX_SDD.txt", Path_Out);  
	if ((fpt_Out = fopen(String, "w")) == NULL) {
		printf("Cannot open file %s\n", String);
		exit(0);
	}
	...
	for (yy = 0; yy < N_Files; yy++) {
		sprintf(String, "%sCLPX_SnowEnd_%d.raw", Path_In, Year[yy]);  //Ob
		if ((fpt_In[yy] = fopen(String, "rb")) == NULL) {
			printf("Cannot open file %s\n", String);
			exit(0);
		}
	...
	for (Line = 0; Line < LINES; Line++) {
		...
		fread(aLine_SDD_0, 4, PIXELS, fpt_In[0]);
		fread(aLine_SDD_1, 4, PIXELS, fpt_In[1]);
		fread(aLine_SDD_2, 4, PIXELS, fpt_In[2]);
		fread(aLine_SDD_3, 4, PIXELS, fpt_In[3]);
		fread(aLine_SDD_4, 4, PIXELS, fpt_In[4]);
		fread(aLine_SDD_5, 4, PIXELS, fpt_In[5]);
		fread(aLine_SDD_6, 4, PIXELS, fpt_In[6]);
		...
		for (Pix = 0; Pix < PIXELS; Pix++) {
			if (aLine_8U[Pix] == 1) {
				...
				fprintf(fpt_Out, "%d %d ", Line, Pix);
				for (yy = 0; yy < N_Files; yy++) fprintf(fpt_Out, "%.0f ", SDD[yy]);
				fprintf(fpt_Out, "\n");
			}
		}  //pix
	}  //line
...
{
	char Path_In_Mask[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026July\\";
	char Path_In_Observed[] = "E:\\Snow_Alaska\\CLPX\\CLPX_30m\\";
	char Path_In[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_SnowDepth_2026July\\";
	char Path_Out[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_SnowDepth_2026July\\";
	...
	//output file
	sprintf(String, "%sSnowDepth_Ob_Ob_precip.txt", Path_Out);  //ERA5, Ob
	if ((fpt_Out = fopen(String, "w")) == NULL) {
		printf("Cannot open file %s\n", String);
		exit(0);
	}
	fprintf(fpt_Out, "Line Pix ");
	for (yy = 0; yy < N_Files; yy++) fprintf(fpt_Out, "%d_Ob ", Year[yy]);
	for (yy = 0; yy < N_Files; yy++) fprintf(fpt_Out, "%d_Ob_precip ", Year[yy]);  //ERA5, Ob
	fprintf(fpt_Out, "\n");
	...
	for (yy = 0; yy < N_Files; yy++) {
//		sprintf(String, "%sCLPX_Ob_precip_SnowDepth_avg_Fsnow_Apr_%d.dat", Path_In, Year[yy]);  //Ob
		sprintf(String, "%sSnowDepth_30m_%d.raw", Path_In_Observed, Year[yy]);
		if ((fpt_In_Ob[yy] = fopen(String, "rb")) == NULL) {
			printf("Cannot open file %s\n", String);
			exit(0);
		}
...
void Output_pixel_values_of_Fsnow_images_from_SnowDepth_for_calculating_regression()
{
	...
	//output file
	sprintf(String, "%sCLPX_ERA5_precip_Fsnow_from_SnowDepth.txt", Path_Out);
	if ((fpt_Out = fopen(String, "w")) == NULL) {
		printf("Cannot open file %s\n", String);
		exit(0);
	}
	...
	for (yy = 0; yy < 6; yy++) {
		sprintf(String, "%sCLPX_ERA5_precip_Fsnow_from_SnowDepth_%d.dat", Path_In, Year[yy]);
...
//output file
	sprintf(String, "%sCLPX_Fsnow_Ob_precip.txt", Path_Out); //Ob, ERA5
	if ((fpt_Out = fopen(String, "w")) == NULL) {
		printf("Cannot open file %s\n", String);
		exit(0);
	}
...
//input files
	...
	for (yy = 0; yy < 7; yy++) {
		sprintf(String, "%sCLPX_Ob_precip_avg_Fsnow_%d.dat", Path_In, Year[yy]);  //Ob, ERA5
		if ((fpt_In_16S[yy] = fopen(String, "rb")) == NULL) {
			printf("Cannot open file %s\n", String);
			exit(0);
		}

		sprintf(String, "%sCLPX_SnowEnd_u_%d.raw", Path_In_SnowEnd_u, Year[yy]);
...
{
	char Common_Path[] = "E:\\Snow_Alaska\\Modelling_2026\\";
	char Path_In_Area[] = "E:\\Snow_Alaska\\CLPX\\CLPX_30m\\";                    //_5m  for spatial input
	char Path_Out_Area[] = "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026May\\";  //_5m_Spatial_Output
	char Clime_Site_Name[] = "CLPX_ERA5_Precip";  //Ob, ERA5
...
//	int Input_Variable = 0; //0 or 2. using this variable to estimate Fsnow: 0:Snowcover_day1, (1:SnowCover_day2), 2:SnowDepth 
	...
	for (Year = 2012; Year <= 2018; Year++) {
		...
		sprintf(Note, "%sCLPX_30m_Landcover.raw", Path_In_Area);  //8U, HV_5m
		//	sprintf(Note, "%sHV_30m_Landcover.raw", Path_In_Area);  //8U, HV_5m
		if ((fpt_LC = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
		}
		sprintf(Note, "%sCLPX_30m_LAI.raw", Path_In_Area);  //16S, LAI*1000,  HV_5m
		if ((fpt_LAI = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
		}
		sprintf(Note, "%sCLPX_30m_ArcticDEM_Slope.raw", Path_In_Area);  //32R, degrees, HV_5m, 30m
		//	sprintf(Note, "%sHV_5m_Slope.raw", Path_In_Area);  //32R, degrees
		if ((fpt_Slope = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
		}
		sprintf(Note, "%sCLPX_30m_ArcticDEM_Aspect.raw", Path_In_Area);  //32R, degrees, HV_5m, 30m
		//	sprintf(Note, "%sHV_5m_Aspect.raw", Path_In_Area);  //32R, degrees
		if ((fpt_Aspect = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
		...
		sprintf(Note, "%sSnowDepth_30m_%d.raw", Path_In_Area, Year);  //32f, Snow endB date
		if ((fpt_SnowCondition = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
		}
...
void Mapping_Fsnow_CLPX()
{
	char Common_Path[]  = "E:\\Snow_Alaska\\Modelling_2026\\";  
	char Path_In_Area[] = "E:\\Snow_Alaska\\CLPX\\CLPX_30m\\";                    //_5m  for spatial input
	char Path_In_SnowEnd[] = "E:\\Snow_Alaska\\Data_From_Mitchell\\2026_YZ\\CLPX_SnowEndB_Layers\\";
	char Path_Out_Area[]= "E:\\Snow_Alaska\\Modelling_2026\\CLPX\\Output_Fsnow_2026July\\";  //_5m_Spatial_Output
	char Clime_Site_Name[] = "CLPX_Ob_Precip";  //Ob, ERA5
...
//	int Input_Variable = 0; //0 or 2. using this variable to estimate Fsnow: 0:Snowcover_day1, (1:SnowCover_day2), 2:SnowDepth 
	...
	for (R = 0; R < 2; R++) Loc_Line[R] = ALL_Loc_Lines[R];
		...
		sprintf(Note, "%s%s_mid_Fsnow_2019_2025_test.dat", Path_Out_Area, Clime_Site_Name);  //
		if ((fpt_Output = fopen(Note, "wb")) == NULL) {
			printf("Cannot open output file %s\n", Note);
			exit(0);
		}
	...
	//Input model results at L1 and L2 ---------------------------------------
		...
		sprintf(Note, "%sCLPX_SnowEnd_%d.raw", Path_In_SnowEnd, Year);  //32f, Snow endB date
		if ((fpt_SnowCondition = fopen(Note, "rb")) == NULL) {
			printf("Cannot open input file %s\n", Note);
			exit(0);
		}

		sprintf(Note, "%sCLPX_SnowEnd_u_%d.raw", Path_In_SnowEnd, Year);  //32f, Snow end u
...
//	double x111[2], x112[2], x121[2], x122[2], x211[2], x212[2];  //for checking
...
//		x111[0] = Loc2_Snow111[Site][0];  //check
//		x111[1] = Loc2_Snow111[Site][1];  //check
...
//		x112[0] = Loc2_Snow112[Site][0];  //check
//		x112[1] = Loc2_Snow112[Site][1];  //check
...
//		x121[0] = Loc2_Snow121[Site][0];  //check
//		x121[1] = Loc2_Snow121[Site][1];  //check
...
//		x122[0] = Loc2_Snow122[Site][0];  //check
//		x122[1] = Loc2_Snow122[Site][1];  //check
...
//find and return the modelled corresponding snow conditions at N_Fsnow values: pLoc2_Snow111[0 or 1][0 to N_Fsnow -1]
		...
		for (Site = 0; Site < Total_Rows; Site++) {
			...
			i2 = i1 + N_Fsnow;  
			for (i = i1; i < i2; i++) 
				Loc2_Snow111[Site][i - i1] = L_Values[i];
...
//	int Output_Variable = Year - 2012;  //for calculating snow depth in April in a Ob year, the first year is 2012
	...
	for (Site = 0; Site < Total_Rows; Site++) {
		...
		if(Total_Rows==1) 	sprintf(Input_File, "%s.txt", File_Name);  //for CLPX
		else                sprintf(Input_File, "%s_%s.txt", File_Name, Site_Name[Site]); //two sites HV
...
//find Fsnow based on Snow condition at the pixel
		if (SnowCondition < Snow_x[0]) {
			...
			P = Snow_x[N_Fsnow_1] - Snow_x[N_Fsnow_2];
			if (P < 1.0) P = 1.0;
			F = (Fsnow_Avg[N_Fsnow_1] - Fsnow_Avg[N_Fsnow_2]) / P;  // (Snow_x[N_Fsnow_1] - Snow_x[N_Fsnow_2]);
